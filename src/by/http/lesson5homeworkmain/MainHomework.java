/*Задание 4 из Блинова: Создать  объект  класса Государство,  используя  классы Область (region), Район (district), Город. 
Методы:  вывести  на консоль  столицу,  количество  областей,  площадь, областные центры*/

/*Алгоритм решения:
1. Создать конструктор в Сity, с помощью которого создадутся города в MainHomework (18 штук)
2. Cоздать конструктор в District, с помощью которого создадутся районы в MainHomework (18 штук)
3. Cоздать конструктор в Region, с помощью которого создадутся области в MainHomework (6 штук)
4. Cоздать конструктор в Country (сразу со столицей сapital), с помощью которого получится Государство (1 штука) 

5. Вывод столицы на консоль: в MainHomework создать метод printCapital <- который вызвается из Сountry <- а в Country лежит 
переменная capital, из который дергается метод getCityName() <- и чтобы он сразуботал надо в City создать геттер getCityName().
Геттер возвращает значение

6. Вывод количества областей (regions): создать в MainHomework массив из областей и вызвать метод setRegions из Country <- а чтобы
засетать области в массив нужно в Country создать геттер и сеттер (также в Country хранится переменная regions []) и потом уже 
в MainHomework вывести на консоль кол-во областей.

7. Площадь: из MainHomework выводим на консоль площадь вызывая метод countArea из Country <- сам метод countArea лежит в Country <-
в методе есть цикл for с оператором присваивания "Добавления" (С+=А это как С=С+А) (также в Country хранится переменная regions []) +
каждому региону мы должны засетать площадь: делаем это в MainHomework, дергаем метод setRegionArea и передаем значение, чтобы
все это работало надо в Regions сгенерить геттер и сеттер

8. Областные центры: из MainHomework выводим на консоль областные центры дергая метод printCenter из Region, в методе цикл в цикле:
цикл использует districts [] (переменная districts [] лежит в Region), переменная cities [] лежит в District, чтобы в массивы
все подтягивалось надо в District and City создать геттеры и сеттеры, делаем в City метод isRegionCenter, делаем в City 
метод setRegionCenter и передаем в него значение true из MainHomework
 */

package by.http.lesson5homeworkmain;

import by.http.lesson5homework.City;
import by.http.lesson5homework.Country;
import by.http.lesson5homework.District;
import by.http.lesson5homework.Region;
